function main() {
    ...
    // -------------------------------------
    // FACTORIAL - implement here
    let cache = {
        1: 1,
        2: 2
    };

    const factorialMemoization = (number) => {
        let cacheResult = cache[number];
        if (cacheResult) {
            return cacheResult;
        } else {
            result = number * factorialMemoization(number - 1);
            cache[number] = result;
            return result;
        }
    }
    ...
    // -------------------------------------
    // FIBONACCI - implement here
    cache = {
        1: 1,
        2: 1
    };

    const fibonacciMemoization = (index) => {
        let cacheValue = cache[index];

        if (cacheValue) {
            return cacheValue;
        } else {
            let result = fibonacciMemoization(index - 1) + fibonacciMemoization(index - 2);
            cache[index] = result;
            return result;
        }
    }
